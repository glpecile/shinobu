...
- **KTD6 — TVDB-vs-TMDB season numbering divergence is accepted risk, symmetrically.** ani.zip's `seasonNumber` is TVDB-derived; Serializd is TMDB-keyed. For anime these overwhelmingly agree; where they don't, **both providers can receive a wrong-but-existing write**: Serializd's `resolveSeasonId` guards only *absent* season numbers (a divergent season that also exists in TMDB resolves and writes verbatim), and Trakt fails loudly only when the season doesn't exist at all (`src/lib/providers/trakt/writes.ts:114-121`, surfacing as a generic error outcome, not a numbering hint). The risk is accepted for both providers on the same rationale — per-episode dataset granularity makes divergence rare — with the follow-up sanity probe (Scope Boundaries) as the trigger if evidence appears. No extra pre-write verification requests in this plan (see Risks).
...
- **Trakt pre-write season sanity check** (episode-count probe before writing) — only if KTD6's accepted numbering-divergence risk materializes in practice.
