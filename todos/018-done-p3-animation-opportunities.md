...
## 6. Confirm a provider connecting

The card now settles into its new section. Connecting still gives no toast or
haptic. A connect succeeds in ten places (the five connect buttons, their native
variants, and the web OAuth returns in `state/session/use-oauth-callback.ts`), so
a `toast.success` needs one shared place first rather than ten call sites.
