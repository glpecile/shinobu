...
/** As many posters as a desktop wall shows above the fold (eight columns, three rows). */
const FIRST_SCREEN_POSTERS = 24;
/** The most a slow image CDN may hold a wall back; past it, posters fill in as they land. */
const WARMUP_CAP_MS = 1200;

function warmPosters(items: NormalizedMediaItem[]): Promise<void> {
  const uris = items
    .slice(0, FIRST_SCREEN_POSTERS)
 ...
 * same `useDeferredValue` that holds the old wall through the data fetch
 * holds it through this too.
 *
...
export function useWarmPosters(
  ...
  useSuspenseQuery({
    ...
    queryFn: () => warmPosters(items).then(() => true),
    staleTime: Infinity,
  });
}
